...
function GameWonMenu:new()
	...
	for index, definition in ipairs(definitions) do
		gameWonMenu[definition.key] = Button:new(
			definition.name,
			BASE_BUTTON_WIDTH,
			BASE_BUTTON_HEIGHT,
	...
	gameWonMenu.PostGameMessage = Text:new(
...
function GameWonMenu:IsPressed(PositionMouse, CursorRadius)
...
function GameWonMenu:HandleMousePressed(x, y, button)
	if button ~= 1 then
		return false
	end
